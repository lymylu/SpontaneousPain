...
for i=3:length(filelist)
      tmpmat=matfile(fullfile(BaseDir,filelist(i).name),'Writable',true);
          % tmpmat=cal_binspikes(tmpmat,50,'Combined',Eventtype);
...
%%
SponDir='optospon';
filelist=dir(SponDir);
for i=3:length(filelist)
    tmpmat=matfile(fullfile(SponDir,filelist(i).name),'Writable',true);
    tmpmat=cal_binspikes(tmpmat,1,'Spon','SPKdata');
end
%%
...
%%
%psthspikesummry_correct=cellfun(@(x) basecorrect(x,pstht,-1,0,'Zscore'),psthspikesummary,'UniformOutput',0);
...
%% diviede the opto neurons into three groups 
...
%         invalid=isnan(mean(optotaggedfiring,2));
    ...
    elseif i==2
        ...
        %tmpindex=negative(optoindex==0&neuroinvalid==1)==1; 
%         invalid=isnan(mean(optomodulatedfiring,2));
        tmpindex=find(tmpindex==1);
        %tmpindex(invalid)=[];
    ...
    elseif i==1
        ...
        %tmpindex=negative(optoindex==0&neuroinvalid==1)==0; 
        %invalid=isnan(mean(optounmodulatedfiring,2));
        tmpindex=find(tmpindex==1);
        %tmpindex(invalid)=[];
        [~,sortindex]=sort(mean(binspikesummary_short_correct{9}(t_short>=0&t_short<0.1,tmpindex),1),2);
       tmp=binspikesummary_short_correct{9}(:,tmpindex);
...
for i=1:3
    ...
    if i==3
       ...
       subplot(2,3,3);
       imagesc(t,1:size(tmp(:,valid),2),smoothdata(tmp(:,sortindex(valid)),1,'gaussian',5)');axis tight; xlim([-0.5,1]);caxis([-1,2]);
    ...
    elseif i==2
       ...
       tmp=binspikesummary_short_correct{9}(:,typeindex{i});
       %valid=[3,4,8,10,12:14,16:32,34:size(tmp,2)];
       imagesc(t,1:size(tmp,2),smoothdata(tmp(:,sortindex),1,'gaussian',5)');axis tight; xlim([-0.5,1]);caxis([-1,2]);
...
for i=1:9
    ...
    eventinvalid=ismember(eventselect,eventinvalid); 
        ...
        lfp{i}=cat(3,lfp{i},nanmean(tmpmat.Spectrom_STFT(:,:,eventindex),3));
    end
end
